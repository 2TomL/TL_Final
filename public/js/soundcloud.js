...
function setupSoundCloudPopout() {
        ...
        if (window.SC && window.SC.Widget && iframe) {
            ...
            const applyVolume = () => {
                try {
                    ...
                    console.warn('SoundCloud volume set failed', err);
                }
            };
            widget.bind(window.SC.Widget.Events.READY, applyVolume);
            applyVolume();
        }
    }
    if (volumeSlider) {
    ...
    function setFallbackTrack() {
        if (iframe) {
            iframe.src = 'https://w.soundcloud.com/player/?url=https%3A%2F%2Fsoundcloud.com%2Fsoundcloud%2Fsets%2Fsounds&color=%23ff5500&auto_play=true&hide_related=false&show_comments=false&show_user=true&show_reposts=false&show_teaser=false&visual=false';
        }
    }
}
